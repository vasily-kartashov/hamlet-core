...
namespace Hamlet\Requests {
    ...
    class RequestBuilderTest extends AbstractRequestTest {
        ...
        protected function createRequestFromPath($path) : Request {
            $requestBuilder = new RequestBuilder();
            return $requestBuilder->setPath($path)->build();
        }

        protected function createRequestFromJSON($json) : Request {
            $requestBuilder = new RequestBuilder();
            return $requestBuilder->parseJSON($json)->build();
        }

        public function testSetPath() {
            $requestBuilder = new RequestBuilder();
            $request = $requestBuilder->setPath('/test?a=1')->build();
